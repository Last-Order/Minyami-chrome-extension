<template>
    <div class="main-container">
        <div>
            <div class="buttons">
                <el-button type="text" @click="showConfig = true">{{
                    $t("message.settings")
                }}</el-button>
                ...
                <el-form :inline="true">
                    <el-form-item>
                        <el-checkbox
                            ...
                            :label="$t('message.resumeMode')"
                        ></el-checkbox>
                    </el-form-item>
                    <el-form-item>
                        <el-checkbox
                            v-model="form.live"
                            :label="$t('message.liveMode')"
                        ></el-checkbox>
                    </el-form-item>
                </el-form>
                <template v-for="chunkList in playlist.chunkLists">
                    <el-form
                        class="playlist-chunklist-item"
                        :key="chunkList.url"
                    >
                        <el-form-item>
                            <div class="playlist-chunklist-item-info">
                                <template v-if="chunkList.type === 'video'">
                                    <el-tag
                                        ...
                                        size="mini"
                                        class="playlist-tag"
                                        >{{ $t("message.video") }}</el-tag
                                    >
                                    <span v-if="chunkList.resolution"
                                        >{{ $t("message.resolution") }}:
                                        {{ chunkList.resolution.x }} ×
                                        {{ chunkList.resolution.y }}</span
                                    >
                                    <span v-if="chunkList.bandwidth"
                                        >{{ $t("message.bitrate") }}:
                                        {{
                                            Math.round(
                                                chunkList.bandwidth / 1024
                                            )
                                        }}
                                        kbps</span
                                    >
    ...
    methods: {
        ...
        generateCommand(chunklist, playlist) {
            ...
            if (!this.form.recoverMode) {
                command += `${prefix} -d "${
                    chunklist.url
                }" --output "${playlist.title.replace(/\"/g, `\\"`)}.ts"`;
            } else {
        ...
        getCookies() {
            chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
                if (tabs[0]) {
                    const cookies = Storage.getHistory(
                        tabs[0].url + "-cookies"
                    );
